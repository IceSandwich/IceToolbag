...
class ICETB_OT_RichStrip_Rebuild(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if data.HasAudio:
            context.scene.sequence_editor.sequences_all.get(("rs%d-audio"%rsid_old)+suffix).name = "%saudio"%newrsid
        context.scene.sequence_editor.sequences_all.get(("rs%d-movie"%rsid_old)+suffix).name = "%smovie"%newrsid
        context.scene.sequence_editor.sequences_all.get(("rs%d-fixfps"%rsid_old)+suffix).name = "%sfixfps"%newrsid
