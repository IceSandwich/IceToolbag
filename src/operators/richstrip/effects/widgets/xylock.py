...
def drawWithExportBox(layout, richstrip, obj_x, x_attr, x_bool:str, obj_y, y_attr, y_bool:str, obj_lock, lock_attr, x_label="X", y_label="Y", union_label="Size"):
    effect = richstrip.IceTB_richstrip_data.getSelectedEffect()
    bxproperty, byproperty = EffectBase.getBoolProperty(effect, x_bool), EffectBase.getBoolProperty(effect, y_bool)
    viewbool = getattr(obj_lock, lock_attr)
