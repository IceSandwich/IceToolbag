...
class ICETB_OT_Marker_BatchRename(bpy.types.Operator):
    ...
    newname_string: bpy.props.StringProperty(name="New Name", default="F_%03d")
    ...
    def execute(self, context):
        if len(self.markers) == 1:
            if self.newname_string == "":
                self.newname_string = "F_%03d" % self.markers[0].frame
            self.markers[0].name = self.newname_string
            return {"FINISHED"}

        if self.newname_string == "":
            self.newname_string = "F_%03d"
        elif re.search('[a-zA-Z0-9\\_]*\\%[0-9]*d', self.newname_string) == None:
            self.newname_string = self.newname_string + "_%03d"
